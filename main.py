...
def ask_and_save_settings():
    ...
    يدعم أيضاً المتغيرات البيئية (env) كبديل عن input() للتشغيل غير التفاعلي:
        API_ID, API_HASH, BOT_TOKEN, DEV_ZAID
    """
    import os as _os
    ...
    if not data.get("api_id"):
        env_val = _os.environ.get("API_ID", "").strip()
        data["api_id"] = env_val if env_val else input("ادخل API_ID : ").strip()
        changed = True

    if not data.get("api_hash"):
        env_val = _os.environ.get("API_HASH", "").strip()
        data["api_hash"] = env_val if env_val else input("ادخل API_HASH : ").strip()
        changed = True

    if not data.get("token"):
        env_val = _os.environ.get("BOT_TOKEN", "").strip()
        data["token"] = env_val if env_val else input("ادخل توكن البوت (BOT TOKEN) : ").strip()
        changed = True

    if not data.get("dev_zaid"):
        env_val = _os.environ.get("DEV_ZAID", "").strip()
        data["dev_zaid"] = env_val if env_val else input("ادخل ايدي المالك (Dev_Zaid) : ").strip()
        changed = True
