...
from helpers.utils import html_escape
...
# ملاحظة: (?!مميز|عام) يمنع التعارض مع "اضف رد مميز" و"اضف رد عام"
@Client.on_message(filters.group & filters.regex(r"^اضف\s+رد\s+(?!مميز|عام)([\s\S]+)$"))
async def add_group_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"✅ تم إضافة رد المجموعة على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def del_group_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"🗑 تم حذف رد المجموعة على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def list_group_filters(client: Client, message: Message):
    ...
    if not kws:
        ...
        return
    text = "📋 <b>ردود المجموعة:</b>\n\n" + "\n".join(f"• <code>{html_escape(k)}</code>" for k in kws)
...
async def add_member_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"✅ تم حفظ ردك الشخصي على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def del_member_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"🗑 تم حذف ردك على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def add_special_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"⭐ تم إضافة رد مميز على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def del_special_filter(client: Client, message: Message):
    ...
    await message.reply_text(f"🗑 تم حذف الرد المميز : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
# group=10 لضمان تنفيذه بعد كل handlers الأوامر (group=0 افتراضي)
# ============================================================
@Client.on_message(filters.group & filters.text & ~filters.via_bot, group=10)
