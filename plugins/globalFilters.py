...
async def add_global(client: Client, message: Message):
    ...
    await message.reply_text(f"🌐 تم إضافة رد عام على : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def del_global(client: Client, message: Message):
    ...
    await message.reply_text(f"🗑 تم حذف الرد العام : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
async def list_global(client: Client, message: Message):
    ...
    if kws:
        lines.append("\n<u>عادية:</u>")
        lines += [f"• <code>{html_escape(k)}</code>" for k in kws]
    if multi_kws:
        lines.append("\n<u>متعددة:</u>")
        lines += [f"• <code>{html_escape(k)}</code>  ({rds.llen(_multi_key(k))})" for k in multi_kws]
...
async def add_global_multi(client: Client, message: Message):
    ...
    await message.reply_text(
        f"🌐➕ تم إضافة رد متعدد على : <b>{html_escape(keyword)}</b>  (عدد الردود: {count})",
...
async def del_global_multi(client: Client, message: Message):
    ...
    rds.delete(key)
    await message.reply_text(f"🗑 تم حذف الرد المتعدد : <b>{html_escape(keyword)}</b>", parse_mode=ParseMode.HTML)
...
# group=11 لضمان تنفيذه بعد محرك ردود المجموعات (group=10)
# ============================================================
@Client.on_message(filters.text & ~filters.via_bot, group=11)
